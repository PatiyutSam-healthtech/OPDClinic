...
## 10. อัปเดต: ส่งออก CSV รายเดือน + คิวรอตรวจบอกลำดับ/เวลารอ + คู่มือในตัวเว็บ

โค้ดล่าสุดเพิ่ม:
- Action ใหม่ `getVisitsByMonth` (อ่านอย่างเดียว ไม่ต้อง admin) สำหรับปุ่มส่งออก
  CSV ในแท็บ "สรุปภาพรวม"
- `getWaitingList` ตอบเพิ่ม `queueNumber` (ลำดับคิว) และ `waitMinutes` (รอมาแล้วกี่
  นาที) ต่อคนไข้ — ไม่กระทบ field เดิมที่มีอยู่แล้ว
- ปุ่ม "วิธีใช้งาน" ในเมนูซ้ายของเว็บ เปิดคู่มือสั้นๆ ในตัวแอปได้เลย (เนื้อหาเดียว
  กับ `USER_MANUAL.md`)

ขั้นตอนอัปเดต (เหมือนรอบก่อนหน้า): เอา `gas/Code.gs` ไปวางทับ → deploy เวอร์ชันใหม่
ให้ deployment ของ API → เอา `index.html` ไป deploy ทับที่ GitHub Pages ไม่ต้องทำ
อะไรเพิ่มเติมอีก (ไม่มี sheet ใหม่ ไม่มี migration)
